...
function getUserPosts($name, $offset) {
	...
	try {
		$conn = new PDO($db, $user, $pass);
		$input = $conn->prepare("SELECT *, DATEDIFF(CURTIME(), date) AS daysAgo, EXTRACT(HOUR FROM CURTIME()) - EXTRACT(HOUR FROM date) AS hoursAgo, EXTRACT(MINUTE FROM CURTIME()) - EXTRACT(MINUTE FROM date) AS minutesAgo FROM post WHERE author=" . $conn->quote($name) . " ORDER BY date DESC LIMIT 25 OFFSET $offset;");
		if (!$input->execute()) {
			echo "Failed to read posts: " . $input->errorInfo()[0] . " " . $input->errorInfo()[2] . "<br>";
		}
